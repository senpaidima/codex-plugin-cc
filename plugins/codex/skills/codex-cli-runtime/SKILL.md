...
- Leave `--effort` unset unless the user explicitly requests a specific effort; the companion runtime uses its pinned `xhigh` default.
- Leave model unset by default so the companion runtime uses its pinned `gpt-5.5` default. Add `--model` only when the user explicitly asks for one.
