...
- Leave `--effort` unset unless the user explicitly requests a specific reasoning effort; the companion runtime uses its pinned `xhigh` default.
- Leave model unset by default so the companion runtime uses its pinned `gpt-5.5` default. Only add `--model` when the user explicitly asks for a specific model.
